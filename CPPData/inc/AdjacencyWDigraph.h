...
public:
	AdjacencyWDigraph(int numberOfVertices = 0, T theNoEdge = 0) { // Constructor.
	...
	bool existsEdge(int i, int j) const { // Return true iff (i,j) is an edge of the graph. 
	...
	void insertEdge(edge<T> *theEdge) { // Insert edge theEdge into the digraph; if the edge is already
	...
	void eraseEdge(int i, int j) {  // Delete the edge (i,j).
		if (i >= 1 && j >= 1 && i <= n && j <= n && a[i][j] != noEdge) {
			a[i][j] = noEdge;
			e--;
		}
	...
	void bfs(int v, int reach[], int label) { // Breadth-first search. reach[i] is set to label for
	...
	void shortestPaths(int sourceVertex, T* distanceFromSource,
			int* predecessor) { // Find shortest paths from sourceVertex.
		...
		while (!newReachableVertices.empty()) {  // more paths exist
...
//	template<class T>
	void allPairs(T **c, int **kay) { // Dynamic programming all pairs shortest paths algorithm.
			...
			for (int i = 1; i <= n; i++)
				for (int j = 1; j <= n; j++)
					if (c[i][k] != noEdge && c[k][j] != noEdge
							&& (c[i][j] == noEdge || c[i][j] > c[i][k] + c[k][j])) { // smaller value for c[i][j] found
						c[i][j] = c[i][k] + c[k][j];
						kay[i][j] = k;
					}
	}

	T btSalesperson(int *bestTour) {   // Traveling salesperson by backtracking.
	...
	T leastCostBBSalesperson(int *bestTour) { // least-cost branch-and-bound code to find a shortest tour
		...
		for (int i = 1; i <= n; i++) { // compute costOfMinOutEdge[i] and sumOfMinCostOutEdges
			T minCost = noEdge;
			for (int j = 1; j <= n; j++)
				if (a[i][j] != noEdge
						&& (minCost == noEdge || minCost > a[i][j]))
					minCost = a[i][j];
		...
		while (eNode.sizeOfPartialTour < n - 1) {         // not at leaf
			partialTour = eNode.partialTour;
			if (eNode.sizeOfPartialTour == n - 2) {         // parent of leaf
				...
				if (a[partialTour[n - 2]][partialTour[n - 1]] != noEdge
						...
						&& (costOfBestTourSoFar == noEdge
								|| eNode.costOfPartialTour
										+ a[partialTour[n - 2]][partialTour[n
												- 1]] + a[partialTour[n - 1]][1]
										< costOfBestTourSoFar)) { // better tour found
				...
				for (int i = eNode.sizeOfPartialTour + 1; i < n; i++)
					if (a[partialTour[eNode.sizeOfPartialTour]][partialTour[i]]
							!= noEdge) {
						// feasible child, bound path cost
						T costOfPartialTour =
								eNode.costOfPartialTour
										+ a[partialTour[eNode.sizeOfPartialTour]][partialTour[i]];
						T minAdditionalCost =
								eNode.minAdditionalCost
										- costOfMinOutEdge[partialTour[eNode.sizeOfPartialTour]];
						T leastCostPossible = costOfPartialTour
								+ minAdditionalCost;
						if (costOfBestTourSoFar == noEdge
								|| leastCostPossible < costOfBestTourSoFar) { // subtree may have better leaf, put root in min heap
							heapNode hNode(leastCostPossible, costOfPartialTour,
									minAdditionalCost,
									eNode.sizeOfPartialTour + 1, new int[n]);
							for (int j = 0; j < n; j++)
...
protected:
	void rTSP(int currentLevel) { // Recursive backtracking code for traveling salesperson.
			...
			if (a[partialTour[n - 1]][partialTour[n]] != noEdge
					...
					&& (costOfBestTourSoFar == noEdge
							|| costOfPartialTour
									...
									+ a[partialTour[n]][1] < costOfBestTourSoFar)) { // better tour found
				...
				if (a[partialTour[currentLevel - 1]][partialTour[j]] != noEdge
						&& (costOfBestTourSoFar == noEdge
								|| costOfPartialTour
										...
										< costOfBestTourSoFar)) { // search this subtree
					swap(partialTour[currentLevel], partialTour[j]);
					costOfPartialTour +=
							a[partialTour[currentLevel - 1]][partialTour[currentLevel]];
					rTSP(currentLevel + 1);
					costOfPartialTour -=
							a[partialTour[currentLevel - 1]][partialTour[currentLevel]];
					swap(partialTour[currentLevel], partialTour[j]);
