...
class BinarySearchTree: public bsTree<K, E>, public LinkBinaryTree<
		pair<const K, E> > {
public:
	// methods of dictionary
	bool empty() const {
		return LinkBinaryTree < pair<const K, E> > ::treeSize == 0;
	}
	int size() const {
		return LinkBinaryTree < pair<const K, E> > ::treeSize;
	}
	...
	void ascend() {
		LinkBinaryTree < pair<const K, E> > ::inOrderOutput();
	}
};
...
pair<const K, E>* BinarySearchTree<K, E>::find(const K& theKey) const { // Return pointer to matching pair.
	...
	binaryTreeNode < pair<const K, E> > *p = LinkBinaryTree < pair<const K, E>
			> ::root;
	while (p != NULL)
		// examine p->element
...
void BinarySearchTree<K, E>::insert(const pair<const K, E>& thePair) { // Insert thePair into the tree. Overwrite existing
	...
	if (LinkBinaryTree < pair<const K, E> > ::root != NULL) // the tree is not empty
		if (thePair.first < pp->element.first)
			pp->leftChild = newNode;
		else
			pp->rightChild = newNode;
	else
		LinkBinaryTree < pair<const K, E> > ::root = newNode; // insertion into empty tree
	LinkBinaryTree < pair<const K, E> > ::treeSize++;
}
...
void BinarySearchTree<K, E>::erase(const K& theKey) { // Delete the pair, if any, whose key equals theKey.
	...
	if (p->leftChild != NULL && p->rightChild != NULL) {   // two children
		...
		if (pp == NULL)
			LinkBinaryTree < pair<const K, E> > ::root = q;
	...
	// delete p
	if (p == LinkBinaryTree < pair<const K, E> > ::root)
		LinkBinaryTree < pair<const K, E> > ::root = c;
