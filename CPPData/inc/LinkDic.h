...
class LinkDic: public Dictionary<K, E> {
public:
	LinkDic() {
		firstNode = NULL;
		dSize = 0;
...
template<class K, class E>
LinkDic<K, E>::~LinkDic() {            // Destructor.  Delete all nodes.
...
template<class K, class E>
pair<const K, E>* LinkDic<K, E>::find(const K& theKey) const { // Return pointer to matching pair.
...
template<class K, class E>
void LinkDic<K, E>::insert(const pair<const K, E>& thePair) { // Insert thePair into the dictionary. Overwrite existing
...
template<class K, class E>
void LinkDic<K, E>::erase(const K& theKey) { // Delete the pair, if any, whose key equals theKey.
	pairNode<K, E> *p = firstNode, *tp = NULL; // tp trails p
...
template<class K, class E>
void LinkDic<K, E>::output(ostream& out) const { // Insert the chain elements into the stream out.
...
// overload <<
template<class K, class E>
ostream& operator<<(ostream& out, const LinkDic<K, E>& x) {
	x.output(out);
	return out;
}
