...
template<class T>
Matrix<T>::Matrix(int theRows, int theColumns) {     // matrix constructor.
...
template<class T>
Matrix<T>& Matrix<T>::operator=(const Matrix<T>& m) { // Assignment. (*this) = m.
...
template<class T>
T& Matrix<T>::operator()(int i, int j) const { // Return a reference to element (i,j).
...
template<class T>
Matrix<T> Matrix<T>::operator+(const Matrix<T>& m) const { // Return w = (*this) + m.
...
template<class T>
Matrix<T> Matrix<T>::operator-(const Matrix<T>& m) const { // Return (*this) - m.
	if (theRows != m.theRows || theColumns != m.theColumns)
		throw matrixSizeMismatch();
...
template<class T>
Matrix<T> Matrix<T>::operator*(const Matrix<T>& m) const { // matrix multiply.  Return w = (*this) * m.
	...
	for (int i = 1; i <= theRows; i++) {   // compute row i of result
		for (int j = 1; j <= m.theColumns; j++) { // compute first term of w(i,j)
...
template<class T>
Matrix<T>& Matrix<T>::operator+=(const T& x) { // Increment all elements of *this by x.
	for (int i = 0; i < theRows * theColumns; i++)
		element[i] += x;
	return *this;
}
...
template<class T>
ostream& operator<<(ostream& out, const Matrix<T>& m) { // Put matrix m into the stream out.
														// One row per line.
...
inline ostream& operator<<(ostream& out, const Matrix<int>& m) { // Put matrix m into the stream out.
	// One row per line.
	int k = 0;  // index into element array
	for (int i = 0; i < m.theRows; i++) {  // do row i
		for (int j = 0; j < m.theColumns; j++)
...
template<class T>
void Matrix<T>::transpose(Matrix<T>& b) { // Set b to be the transpose of *this.
