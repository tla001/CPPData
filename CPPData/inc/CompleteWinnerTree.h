...
template<class T>
void CompleteWinnerTree<T>::initialize(T *thePlayer, int theNumberOfPlayers) { // Create winner tree for thePlayer[1:numberOfPlayers].
	...
	if (n % 2 == 1) {  // special case for odd n, play internal and exteral node
...
template<class T>
void CompleteWinnerTree<T>::play(int p, int leftChild, int rightChild) { // play matches beginning at tree[p]
...
template<class T>
void CompleteWinnerTree<T>::rePlay(int thePlayer) { // Replay matches for player thePlayer.
	...
	if (matchNode == n - 1 && n % 2 == 1) {
		...
		tree[matchNode] =
				(player[tree[n - 1]] <= player[lowExt + 1]) ?
						tree[n - 1] : lowExt + 1;
	}
...
void CompleteWinnerTree<T>::output() const {
	cout << "number of players  = " << numberOfPlayers << " lowExt = " << lowExt
			<< " offset = " << offset << endl;
