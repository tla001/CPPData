...
public:
	...
	virtual void bfs(int v, int reach[], int label) { // Breadth-first search. reach[i] is set to label for
	...
	int* findPath(int theSource, int theDestination) { // Find a path from theSource to theDestination. Return the
	...
	bool connected() {               // Return true iff graph is connected.
	...
	int labelComponents(int c[]) { // Label the components of an undirected graph.
	...
	bool topologicalOrder(int *theOrder) { // Return false iff the digraph has no topological order.
		...
		for (int i = 1; i <= n; i++) {         // edges out of vertex i
		...
		while (!stack.empty()) {  // select from stack
			...
			int u;
			while ((u = iNextVertex->next()) != 0) { // visit an adjacent vertex of nextVertex
				inDegree[u]--;
				if (inDegree[u] == 0)
					stack.push(u);
			}
	...
	int bipartiteCover(int *theLabel, int *theCover) { // Return -1 if the bipartite graph has no cover.
	...
	bool kruskal(weightedEdge<T> *spanningTreeEdges) { // Find a min cost spanning tree using Kruskal's method.
		...
		for (int i = 1; i <= n; i++) {        // get all edges incident to i
			vertexIterator<T> *ii = iterator(i);
			int j;
			T w;
			while ((j = ii->next(w)) != 0)
		...
		while (e > 0 && k < n - 1) { // spanning tree not complete & edges remain
			weightedEdge<T> x = heap.top();
			heap.pop();
			e--;
			int a = uf.find(x.vertex1());
			int b = uf.find(x.vertex2());
			if (a != b) {  // select edge x
	...
	void bellmanFord(int s, T *d, int *p) { // Bellman and Ford algorithm to find the shortest paths from vertex s.
		...
		for (int k = 1; k < n; k++) {
			...
			for (ArrayList<int>::iterator iList1 = list1->begin();
					iList1 != list1->end(); ++iList1) { // update d for the neighbors v of vertex u = *iList1
				int u = *iList1;
				...
				while ((v = iu->next(w)) != 0) {
					if (p[v] == 0 || d[u] + w < d[v]) {
						...
						if (!inList2[v]) {                 // put at end of list
...
protected:
	...
	void rDfs(int v) {   // Recursive dfs method.
		reach[v] = label;
		vertexIterator<T> *iv = iterator(v);
		int u;
		while ((u = iv->next()) != 0)
	...
	bool rFindPath(int s) { // Real path finder, performs a depth-first search from s.
							// s should not equal the destination.
							// Return true iff a path to destination is found.
		reach[s] = 1;
		vertexIterator<T>* is = iterator(s);
		int u;
		while ((u = is->next()) != 0) {       // visit an adjacent vertex of s
			if (reach[u] == 0)   // u is an unreached vertex
					{   // move to vertex u
	...
	void createBins(int b, int n) {    // Create b empty bins and n nodes.
		bin = new int[b + 1];
		fill(bin, bin + b + 1, 0);
		node = new binNode[n + 1];
	}

	void insertBins(int b, int v) {    // Insert v into bin b unless b is zero.
		if (b == 0)
	...
	void moveBins(int bMax, int toBin, int v) { // Move vertex v from its current bin to bin toBin.
